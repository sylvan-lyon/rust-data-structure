...
impl CapacityIncrement for DefaultIncrement {
    fn appropriate_size(&mut self, _elem: Layout, curr: usize) -> usize {
        match curr {
            0 => 4,
            x if x <= 1024 => 2 * x,
            x => x + x / 2,
...
{
    #[inline]
    pub fn new_in(alloc: A, cap_calc: C) -> Self {
        Self {
    ...
    #[inline]
    /// actually it allocate exactly capacity + 1 chunks of memory
    pub fn with_capacity_exact_in(cap: usize, alloc: A, cap_calc: C) -> Self {
        let layout = Layout::array::<T>(cap + 1).expect("allocation failed for layout");
        let buf = alloc.allocate(layout).expect("alloc error").as_ptr() as *mut _;
    ...
    pub fn push(&mut self, value: T) {
        if self.is_full() {
            let new_cap = self.cap_calc.appropriate_size(Layout::new::<T>(), self.cap);
    ...
    fn grow_to(&mut self, new_cap: usize) {
        ...
        let new_buf = self
            .alloc
            .allocate(Layout::array::<T>(new_cap).expect("new layout ok"))
            .expect("mem grow ok")
            .as_ptr() as *mut T;

        // if it has elements, we copy them
        if !self.buf.is_null() {
